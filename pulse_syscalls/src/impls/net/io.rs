...
fn resolve_unix_addr(addr: usize, addrlen: usize) -> Result<core::net::SocketAddr, LinuxError> {
    ...
    let path = if addrlen <= 2 {
        ...
        if first_byte == 0 {
            let len = (addrlen as usize).saturating_sub(3).min(108);
            let mut buf = [0u8; 108];
            crate::impls::utils::read_user_bytes(addr + 3, &mut buf[..len])?;
            let name = alloc::string::String::from_utf8_lossy(&buf[..len]);
