...
pub fn write_unix_addr(path_opt: Option<alloc::string::String>, dst: usize, addrlen: usize) -> Result<(), LinuxError> {
    ...
    // Optimization: Avoid dynamic allocation by replacing vec![0u8; 2] and .extend_from_slice with a fixed-size stack array
    ...
    if let Some(path) = path_opt {
        if path.starts_with('\0') {
            ...
            let copy_len = path_bytes.len().min(128 - current_len);
            bytes[current_len..current_len + copy_len].copy_from_slice(&path_bytes[..copy_len]);
            current_len += copy_len;
        } else {
            // Pathname socket
            let path_bytes = path.as_bytes();
            let copy_len = path_bytes.len().min(128 - current_len - 1);
            bytes[current_len..current_len + copy_len].copy_from_slice(&path_bytes[..copy_len]);
            current_len += copy_len;
            bytes[current_len] = 0; // Null terminator
            current_len += 1;
        }
    }
    
    let copy_len = (alen as usize).min(current_len);
    if dst == 0 {
        return Err(LinuxError::EFAULT);
    }
    crate::impls::utils::write_user_bytes(dst, &bytes[..copy_len])?;
    
    let out_len = current_len as u32;
