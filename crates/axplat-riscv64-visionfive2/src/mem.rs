...
const CCACHE_FLUSH64_OFFSET: usize = 0x200;
...
impl MemIf for MemIfImpl {
    ...
    fn phys_ram_ranges() -> &'static [RawRange] {
        ...
        assert!(
            !ranges.is_empty(),
            "VisionFive 2 RAM was queried before the firmware DTB was parsed"
        );
        ranges
    }
    ...
    fn flush_dcache_range(paddr: PhysAddr, size: usize) {
        ...
        // The U74 cache controller flushes a physical cache line from both
        // L1 and L2. This resolves direct-map versus user-map cache aliases.
        let flush64 = (PHYS_VIRT_OFFSET + CCACHE_BASE + CCACHE_FLUSH64_OFFSET) as *mut u64;
        let mut line = paddr.as_usize() & !(CCACHE_LINE_SIZE - 1);
        unsafe {
            asm!("fence iorw, iorw", options(nostack, preserves_flags));
        }
        loop {
            unsafe {
                write_volatile(flush64, line as u64);
            }
            if end - line <= CCACHE_LINE_SIZE {
                break;
            }
...
pub(super) fn init_from_fdt(fdt: &Fdt<'_>, dtb_paddr: usize) {
    ...
    for memory in fdt
        .all_nodes()
        .filter(|node| node.find_property_str("device_type") == Some("memory"))
    {
        if let Some(regions) = memory.reg() {
            for region in regions {
                let Ok(start) = usize::try_from(region.address) else {
                    continue;
                };
                let Some(size) = region.size.and_then(|size| usize::try_from(size).ok()) else {
                    continue;
                };
