    ...
    fn enqueue_at(&mut self, task: Arc<EEVDFTask<T>>, reason: SchedEnqueueReason, now_ns: u64) {
        ...
        if task.is_rt() {
            ...
            let keep_slice = task.is_round_robin()
                && reason == SchedEnqueueReason::Preempt
                && task.rt_remaining.load(Ordering::Acquire) > 0;
            if reason == SchedEnqueueReason::Preempt && (!task.is_round_robin() || keep_slice) {
...
mod tests {
    ...
    fn normal_selection_prefers_eligible_earliest_deadline() {
        ...
        let selected = scheduler.pick_next_task_at(0).unwrap();
    ...
    fn normal_tree_promotes_at_eligibility_boundary() {
        ...
        let first = scheduler.pick_next_task_at(0).unwrap();
        ...
        let second = scheduler.pick_next_task_at(0).unwrap();
        ...
        let third = scheduler.pick_next_task_at(0).unwrap();
    ...
    fn normal_tree_removes_tasks_from_each_partition() {
        ...
        assert!(scheduler.remove_task(&eligible).is_some());
        assert!(!eligible.normal_eligible_link.is_linked());
        assert!(scheduler.remove_task(&ineligible).is_some());
        assert!(!ineligible.normal_ineligible_link.is_linked());
        assert!(scheduler.is_empty());
    ...
    fn normal_pick_releases_intrusive_link_for_reenqueue() {
        ...
        scheduler.enqueue_task(selected, SchedEnqueueReason::Preempt, 0);
        assert_eq!(Arc::strong_count(&first), 2);
        assert_normal_aggregate_matches_scan(&mut scheduler);

        let selected_again = scheduler.pick_next_task_at(0).unwrap();
    ...
    fn preemption_cache_invalidates_at_boundary_and_queue_mutation() {
        ...
        assert!(scheduler.remove_task(&new_earliest).is_some());
        assert!(scheduler.normal_preemption_cache.is_none());
        assert!(scheduler.has_eligible_preemption(350));
    ...
    fn preemption_timer_query_does_not_mutate_scheduler_state() {
        ...
        assert!(scheduler.normal_preemption_cache.is_none());
        assert_eq!(scheduler.next_preemption_deadline(&current, 0), Some(1_000));
        assert_eq!(
        ...
        assert!(scheduler.task_tick_at(&current, 500));
        assert!(candidate.normal_eligible_link.is_linked());
    }
    ...
    fn wakeup_preemption_does_not_require_timer_query_side_effects() {
        ...
        assert!(scheduler.should_preempt(&current, &candidate, 0));
        assert_eq!(scheduler.next_preemption_deadline(&current, 0), Some(1_000));
        assert!(scheduler.normal_preemption_cache.is_none());
        assert_eq!(scheduler.normal_eligibility_scans, 0);
    }
    ...
    fn preemption_timer_uses_slice_deadline_without_eligibility_probe() {
        ...
        assert_eq!(scheduler.next_preemption_deadline(&current, 0), Some(2_495));
        assert_eq!(current.vruntime(), 0);
        assert!(scheduler.normal_preemption_cache.is_none());
        assert!(scheduler.task_tick_at(&current, 1_248));
    }
    ...
    fn preemption_timer_tracks_current_request_deadline() {
        ...
        assert_eq!(scheduler.next_preemption_deadline(&current, 0), Some(700));
        assert!(scheduler.normal_preemption_cache.is_none());
    }
    ...
    fn preemption_timer_ignores_cached_eligibility_boundary() {
        ...
        assert_eq!(scheduler.next_preemption_deadline(&current, 0), Some(1_000));
        assert!(scheduler.task_tick_at(&current, 800));
    }
    ...
    fn normal_aggregate_tracks_queue_lifecycle() {
        ...
        enqueue(&mut scheduler, normal.clone(), SchedEnqueueReason::Spawn, 0);
        enqueue(&mut scheduler, heavy.clone(), SchedEnqueueReason::Spawn, 0);
        enqueue(&mut scheduler, light.clone(), SchedEnqueueReason::Spawn, 0);
        assert_normal_aggregate_matches_scan(&mut scheduler);
        ...
        scheduler.task_started(&running, 0);
        scheduler.task_tick_at(&running, 400);
        assert_normal_aggregate_matches_scan(&mut scheduler);

        scheduler.task_stopped(&running, 800);
        scheduler.enqueue_task(running, SchedEnqueueReason::Preempt, 800);
        assert_normal_aggregate_matches_scan(&mut scheduler);
        ...
        scheduler.enqueue_task(detached, SchedEnqueueReason::Migration, 900);
        assert_normal_aggregate_matches_scan(&mut scheduler);

        assert!(scheduler.set_priority_at(&light, 50, 900));
        assert_normal_aggregate_matches_scan(&mut scheduler);
        assert!(scheduler.set_priority_at(&light, -100, 900));
        assert_normal_aggregate_matches_scan(&mut scheduler);

        assert!(scheduler.remove_task(&normal).is_some());
        assert_normal_aggregate_matches_scan(&mut scheduler);
    }
    ...
    fn current_normal_to_rt_preserves_virtual_time_for_new_normal_work() {
        ...
        let current = scheduler.pick_next_task_at(0).unwrap();
        scheduler.task_started(&current, 0);
        enqueue(&mut scheduler, peer, SchedEnqueueReason::Spawn, 0);

        assert!(scheduler.set_priority_at(&current, 50, 1_000));
    ...
    fn single_task_deadline_query_defers_runtime_accounting() {
        ...
        scheduler.task_started(&current, 0);
        assert_eq!(scheduler.next_preemption_deadline(&current, 250), None);
        assert_eq!(current.vruntime(), 0);
    ...
    fn fifo_rt_task_does_not_rotate_with_equal_priority_peer() {
        ...
        assert_eq!(scheduler.next_preemption_deadline(&current, 0), None);
        assert!(!scheduler.task_tick_at(&current, RT_TIME_SLICE_NS));
        assert_eq!(
    ...
    fn fifo_runtime_does_not_charge_normal_vruntime() {
        ...
        assert!(!scheduler.task_tick_at(&current, 1_000));
        assert_eq!(current.vruntime(), 0);
    }
    ...
    fn fifo_to_round_robin_starts_a_fresh_slice() {
        ...
        assert_eq!(
            scheduler.next_preemption_deadline(&current, 1_000),
            Some(1_000 + RT_TIME_SLICE_NS)
        );
        assert!(!scheduler.task_tick_at(&current, 1_000 + RT_TIME_SLICE_NS - 1));
        assert!(scheduler.task_tick_at(&current, 1_000 + RT_TIME_SLICE_NS));
    }
    ...
    fn fifo_rt_task_rejoins_head_after_higher_priority_preemption() {
        ...
        enqueue(&mut scheduler, higher.clone(), SchedEnqueueReason::Spawn, 1);
        assert!(scheduler.should_preempt(&current, &higher, 1));
        enqueue(
            &mut scheduler,
            current.clone(),
            SchedEnqueueReason::Preempt,
        ...
        assert!(Arc::ptr_eq(
            &scheduler.pick_next_task_at(1).unwrap(),
            &higher
        ));
        assert!(Arc::ptr_eq(
            &scheduler.pick_next_task_at(1).unwrap(),
