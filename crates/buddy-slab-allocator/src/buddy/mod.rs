...
impl<const PAGE_SIZE: usize> BuddyAllocator<PAGE_SIZE> {
    ...
    pub fn alloc_pages(&mut self, count: usize, align: usize) -> AllocResult<usize> {
        ...
        while !section.is_null() {
            ...
            if let Ok(addr) = Self::alloc_from_section_aligned(current, order, align) {
                let pfn = (addr - current.heap_start) / PAGE_SIZE;
                Self::trim_allocation_to_count(current, pfn, count, order);
                return Ok(addr);
            }
    ...
    /// Allocate exactly `count` pages starting at `addr`.
    ///
    /// Unlike a normal buddy request, the fixed range may span multiple buddy
    /// blocks. The operation is transactional: already reserved blocks are
    /// returned if any later block in the range is unavailable.
    pub fn alloc_pages_at(
        ...
        let rounded = count
            .checked_next_power_of_two()
            .ok_or(AllocError::InvalidParam)?;
        if rounded.trailing_zeros() as usize > MAX_ORDER {
            return Err(AllocError::InvalidParam);
        }
        ...
        let end_pfn = start_pfn
            .checked_add(count)
            .ok_or(AllocError::InvalidParam)?;
        if end_pfn > section.max_pages {
            return Err(AllocError::InvalidParam);
        }

        let mut pfn = start_pfn;
        while pfn < end_pfn {
            let order = Self::range_block_order(pfn, end_pfn - pfn);
            if Self::alloc_block_at(section, pfn, order).is_err() {
                Self::dealloc_exact_range(section, start_pfn, pfn - start_pfn);
    ...
    #[inline]
    fn range_block_order(pfn: usize, remaining: usize) -> usize {
        let length_order = (usize::BITS - 1 - remaining.leading_zeros()) as usize;
        let alignment_order = if pfn == 0 {
            MAX_ORDER
        } else {
            pfn.trailing_zeros() as usize
        };
        length_order.min(alignment_order).min(MAX_ORDER)
    }
    ...
    fn mark_exact_range(
        ...
        let end = pfn + count;
        while pfn < end {
            let order = Self::range_block_order(pfn, end - pfn);
            unsafe {
                let meta = &mut *section.meta.add(pfn);
                meta.flags = flags;
    ...
    fn dealloc_exact_range(section: &mut BuddySection, mut pfn: usize, count: usize) {
        let end = pfn + count;
        while pfn < end {
            let order = unsafe { (*section.meta.add(pfn)).order as usize };
            Self::dealloc_in_section(section, pfn, order);
            pfn += 1 << order;
        }
    }

    fn trim_allocation_to_count(
        section: &mut BuddySection,
        pfn: usize,
        count: usize,
        allocated_order: usize,
    ) {
        let allocated_pages = 1usize << allocated_order;
        debug_assert!(count <= allocated_pages);
        Self::mark_exact_range(section, pfn, count, PageFlags::Allocated);

        let excess = allocated_pages - count;
        if excess != 0 {
            let excess_pfn = pfn + count;
            Self::mark_exact_range(section, excess_pfn, excess, PageFlags::Allocated);
            Self::dealloc_exact_range(section, excess_pfn, excess);
        }
    }

    fn free_list_contains(section: &BuddySection, pfn: usize, order: usize) -> bool {
        let mut current = section.free_lists[order];
        while current != PFN_NONE {
            if current as usize == pfn {
                return true;
            }
            current = unsafe { (*section.meta.add(current as usize)).next };
        }
        false
    }

    fn alloc_block_at(section: &mut BuddySection, target_pfn: usize, order: usize) -> AllocResult {
        for search_order in order..=MAX_ORDER {
            let block_pfn = target_pfn & !((1usize << search_order) - 1);
            if block_pfn + (1usize << search_order) > section.max_pages
                || !Self::free_list_contains(section, block_pfn, search_order)
            {
                continue;
            }
            ...
            while current_order > order {
                current_order -= 1;
                let right_pfn = current_pfn + (1usize << current_order);
                let (next_pfn, free_pfn) = if target_pfn >= right_pfn {
                    (right_pfn, current_pfn)
                } else {
                    (current_pfn, right_pfn)
                };
                unsafe {
                    let meta = &mut *section.meta.add(free_pfn);
                    meta.flags = PageFlags::Free;
            ...
            debug_assert_eq!(current_pfn, target_pfn);
            unsafe {
                let meta = &mut *section.meta.add(target_pfn);
    ...
    pub fn alloc_pages_lowmem(&mut self, count: usize, align: usize) -> AllocResult<usize> {
        ...
        while !section.is_null() {
            let current = unsafe { &mut *section };
            if let Ok(addr) = Self::alloc_lowmem_from_section(current, order, align, count) {
                let pfn = (addr - current.heap_start) / PAGE_SIZE;
                Self::trim_allocation_to_count(current, pfn, count, order);
                return Ok(addr);
            }
    ...
    /// the original request. Non-power-of-two allocations are represented as a
    /// sequence of allocated buddy blocks and are released as one exact range.
    pub fn dealloc_pages(&mut self, addr: usize, count: usize) {
        ...
        let start_pfn = (addr - section.heap_start) / PAGE_SIZE;
        let Some(end_pfn) = start_pfn.checked_add(count) else {
            debug_assert!(false, "dealloc_pages range overflow");
            return;
        };
        if count == 0 || end_pfn > section.max_pages {
            debug_assert!(false, "dealloc_pages range outside section");
            return;
        }

        // Validate the entire range before changing any free list so a bad
        // caller cannot leave a partially freed allocation behind.
        let mut pfn = start_pfn;
        while pfn < end_pfn {
            let stored = unsafe { &*section.meta.add(pfn) };
            if stored.flags != PageFlags::Allocated && stored.flags != PageFlags::Slab {
                debug_assert!(false, "dealloc_pages called on non-allocated block");
                return;
            }
            let block_pages = 1usize << stored.order;
            if pfn + block_pages > end_pfn {
                debug_assert!(false, "dealloc_pages count does not match allocation");
                return;
            }
            pfn += block_pages;
        }
