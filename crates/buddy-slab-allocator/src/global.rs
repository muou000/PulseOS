...
impl<const PAGE_SIZE: usize> GlobalAllocator<PAGE_SIZE> {
    ...
    /// Allocate contiguous pages at an exact virtual address.
    pub fn alloc_pages_at(&self, addr: usize, count: usize, align: usize) -> AllocResult<usize> {
        self.buddy.lock().alloc_pages_at(addr, count, align)
    }

    /// Allocate independent order-0 pages while acquiring the buddy lock once.
    ///
    /// Returns the number of initialized entries in `pages`.
    pub fn alloc_page_batch(&self, pages: &mut [usize]) -> usize {
        let mut buddy = self.buddy.lock();
        let mut allocated = 0;
        for page in pages {
            let Ok(addr) = buddy.alloc_pages(1, PAGE_SIZE) else {
                break;
            };
            *page = addr;
            allocated += 1;
