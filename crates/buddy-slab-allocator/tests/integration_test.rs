...
#[test]
fn buddy_non_power_of_two_requests_use_exact_page_counts() {
    let heap_size = 64 * PAGE_SIZE;
    let mut region = HostRegion::new(buddy_region_size(heap_size), PAGE_SIZE * 8);
    let mut buddy = BuddyAllocator::<PAGE_SIZE>::new();
    let _section = init_buddy_with_heap_alignment(&mut buddy, &mut region, PAGE_SIZE * 4);
    let initial_free = buddy.free_pages();

    let three = buddy.alloc_pages(3, PAGE_SIZE * 4).unwrap();
    assert_eq!(three % (PAGE_SIZE * 4), 0);
    assert_eq!(buddy.free_pages(), initial_free - 3);

    let five = buddy.alloc_pages(5, PAGE_SIZE).unwrap();
    assert_eq!(buddy.free_pages(), initial_free - 8);

    buddy.dealloc_pages(five, 5);
    buddy.dealloc_pages(three, 3);
    assert_eq!(buddy.free_pages(), initial_free);
}

#[test]
fn buddy_fixed_range_allocation_is_exact_and_transactional() {
    let heap_size = 64 * PAGE_SIZE;
    let mut region = HostRegion::new(buddy_region_size(heap_size), PAGE_SIZE);
    let mut buddy = BuddyAllocator::<PAGE_SIZE>::new();
    let section = init_buddy(&mut buddy, &mut region);
    let target = section.start + 16 * PAGE_SIZE;
    let initial_free = buddy.free_pages();

    let blocker = buddy
        .alloc_pages_at(target + 2 * PAGE_SIZE, 1, PAGE_SIZE)
        .unwrap();
    let before_failed_range = buddy.free_pages();
    assert_eq!(
        buddy.alloc_pages_at(target, 3, PAGE_SIZE),
        Err(AllocError::NoMemory)
    );
    assert_eq!(buddy.free_pages(), before_failed_range);

    buddy.dealloc_pages(blocker, 1);
    assert_eq!(buddy.alloc_pages_at(target, 3, PAGE_SIZE), Ok(target));
    assert_eq!(buddy.free_pages(), initial_free - 3);
    assert_eq!(
        buddy.alloc_pages_at(target, 1, PAGE_SIZE),
        Err(AllocError::NoMemory)
    );

    buddy.dealloc_pages(target, 3);
    assert_eq!(buddy.free_pages(), initial_free);
}
...
fn global_allocated_bytes_changes_with_large_alloc() {
    ...
    let ptr = allocator.alloc(layout).unwrap();
    assert_eq!(allocator.allocated_bytes(), 3 * PAGE_SIZE);
