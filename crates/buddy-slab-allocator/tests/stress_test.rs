...
#[test]
#[ignore = "stress test"]
fn stress_multithread_exact_page_ranges() {
    let mut region = HostRegion::new(HEAP_SIZE, PAGE_SIZE * 16);
    let allocator = GlobalAllocator::<PAGE_SIZE>::new();
    let _ctx = init_global(&allocator, &mut region, WORKERS);
    let baseline = count_free_pages(&allocator);
    let allocator = &allocator;
    let barrier = Barrier::new(WORKERS);
    let live_ranges = Mutex::new(Vec::<(usize, usize, u8)>::new());
    ...
    thread::scope(|scope| {
        for cpu in 0..WORKERS {
            ...
            scope.spawn(move || {
                ...
                let mut rng = seeded_rng(0x2800 + cpu as u64);
                let mut owned = Vec::<(usize, usize, u8)>::new();
                for sequence in 0..4_000usize {
                    if owned.is_empty() || rng.random_bool(0.62) {
                        let count = rng.random_range(1..=31usize);
                        let align_pages = 1usize << rng.random_range(0..=4usize);
                        let align = align_pages * PAGE_SIZE;
                        let Ok(addr) = allocator.alloc_pages(count, align) else {
                            continue;
                        };
                        assert_eq!(addr % align, 0);

                        let end = addr.checked_add(count * PAGE_SIZE).unwrap();
                        let tag = ((cpu << 5) ^ sequence).wrapping_add(1) as u8;
                        {
                            let mut all = live_ranges.lock().unwrap();
                            for &(other_addr, other_count, other_tag) in all.iter() {
                                let other_end = other_addr + other_count * PAGE_SIZE;
                                assert!(
                                    end <= other_addr || other_end <= addr,
                                    "overlapping live ranges: {addr:#x}..{end:#x} tag {tag:#x}, \
                                     {other_addr:#x}..{other_end:#x} tag {other_tag:#x}",
                                );
                            }
                            all.push((addr, count, tag));
                        }

                        for page in 0..count {
                            unsafe {
                                let base = (addr + page * PAGE_SIZE) as *mut u8;
                                base.write_volatile(tag);
                                base.add(PAGE_SIZE - 1).write_volatile(tag);
                            }
                        }
                        owned.push((addr, count, tag));
                    } else {
                        let idx = rng.random_range(0..owned.len());
                        let (addr, count, tag) = owned.swap_remove(idx);
                        for page in 0..count {
                            unsafe {
                                let base = (addr + page * PAGE_SIZE) as *const u8;
                                assert_eq!(base.read_volatile(), tag);
                                assert_eq!(base.add(PAGE_SIZE - 1).read_volatile(), tag);
                            }
                        }
                        {
                            let mut all = live_ranges.lock().unwrap();
                            let global_idx = all
                                .iter()
                                .position(|entry| *entry == (addr, count, tag))
                                .unwrap();
                            all.swap_remove(global_idx);
                        }
                        allocator.dealloc_pages(addr, count);
                    }
                }

                for (addr, count, tag) in owned {
                    for page in 0..count {
                        unsafe {
                            let base = (addr + page * PAGE_SIZE) as *const u8;
                            assert_eq!(base.read_volatile(), tag);
    ...
    assert!(live_ranges.lock().unwrap().is_empty());
    assert_eq!(count_free_pages(allocator), baseline);
}
