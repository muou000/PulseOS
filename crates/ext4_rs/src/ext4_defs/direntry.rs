...
impl Ext4DirEntry {
    ...
    pub fn ext4_dir_get_csum(&self, s: &Ext4Superblock, blk_data: &[u8], ino_gen: u32) -> u32 {
        ...
        let block_data_size = blk_data.len() - core::mem::size_of::<Ext4DirEntryTail>();
        csum = ext4_crc32c(csum, &blk_data[..block_data_size], block_data_size.try_into().unwrap());
        csum
    }
    ...
    pub fn copy_to_slice(&self, array: &mut [u8], offset: usize) {
        ...
        let count = size_of::<Ext4FakeDirEntry>() + self.name_len as usize;
        unsafe {
